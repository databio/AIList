...
{   //split B into two part: n0=20 is the best
    ...
    uint32_t tt;
    if(nB<=n0){
        memcpy(B1, B, nB*sizeof(struct g_data));
        nB1=nB;
    }
    else{
        for(i=0;i<nB-n0;i++){
...
{   //split B into two parts: n0=20 is the best
    ...
    for(i=0;i<24;i++){
        qsort(B[i], nB[i], sizeof(struct g_data), compare_rstart);                
        B1[i] = malloc(nB[i]*sizeof(struct g_data));
        idx[i] = malloc(nB[i]*sizeof(int)); //store the index of B1: i2 = i-i1-1
        B2[i] = malloc(MAX(nB[i]/4, n0)*sizeof(struct g_data));    //nB2<nB/4  
        if(nB[i]<=n0){
            memcpy(B1[i], B[i], nB[i]*sizeof(struct g_data));
            nB1[i] = nB[i];         
    ...
    while (fgets(buf, 1024, fd)) {
        ...
        if(ichr>=0){
            ...
            //BSearch takes very little time!!! 2M bSearch(50M)~0.5s!!!     
            tE = bSearch3a(B[ichr], nB[ichr], qe); 
            //t1 = bSearch3a(B1[ichr], nB1[ichr], qe); 
            //t2 = bSearch3a(B2[ichr], nB2[ichr], qe);              
            if(tE>=0){              
                t = idx[ichr][tE];
                t1 = tE - t -1;
                while(t>=0 && aiList1[ichr][t]>qs){
                ...
                if(nB2[ichr]>0){     
                    while(t1>=0 && aiList2[ichr][t1]>qs){
                        if(B2[ichr][t1].r_end>qs){
                            if(qhits<bufsize){
                                OlsC[qhits] = ichr;
                                OlsI[qhits] = t1;                
                            }                  
