...
{   //split B into two parts: n0=20 is the best
    ...
    printf("Constructing time: %f\n", ((double)(end1-start1))/CLOCKS_PER_SEC);    
    //-------------------------------------------------------------------------
    //uint32_t* Ols = malloc(MAX(nA,nB)*2*sizeof(uint32_t)); 
    ...
    while (fgets(buf, 1024, fd)) {
        ...
        if(ichr>=0){
            qhits = 0;
            qs = (uint32_t)atol(s2);
            qe = (uint32_t)atol(s3) - 1;   
            //-----------------------------------------------------------------
            tE = bSearch3(B1[ichr], nB1[ichr], qe);                 
            t = tE-1;
            while(t>=0 && aiList1[ichr][t]>=qs){
                if(B1[ichr][t].r_end>=qs){
                    qhits++;//Total++;
                    //printf("%s\t%u\t%u\n", s1, B1[t].r_start-gs, B1[t].r_end-gs);
                }    
                t--;
            }
            if(nB2[ichr]>16){
                tE = bSearch3(B2[ichr], nB2[ichr], qe);                 
                t = tE-1;
                while(t>=0 && aiList2[ichr][t]>=qs){
                    if(B2[ichr][t].r_end>=qs){
                        qhits++;//Total++;
                        //printf("%s\t%u\t%u\n", s1, B2[t].r_start-gs, B2[t].r_end-gs);
                    }    
                    t--;
                }
            }
...
{   //split B into two parts: n0=20 is the best
    ...
    int n0=20, dist, i, i1, j, t, tS, tE; 
    uint32_t tt;   
    AInterval** B1 = malloc(24*sizeof(AInterval*)); 
    AInterval** B2 = malloc(24*sizeof(AInterval*));       
    int *nB1 = calloc(24, sizeof(int));
    int *nB2 = calloc(24, sizeof(int));      
    for(i=0;i<24;i++){
        qsort(B[i], nB[i], sizeof(struct g_data), compare_rstart);                
        B1[i] = malloc(nB[i]*sizeof(AInterval));
        B2[i] = malloc(MAX(nB[i]/4, n0)*sizeof(AInterval));    //nB2<nB/4  
        if(nB[i]<=n0){
            B1[i] = B[i];
            nB1[i] = nB[i];         
        }
