...
{ 
    ...
    while (fgets(buf, 1024, fd)) {
        ...
        if(ichr>=0){
            ...
            pD[nD].r_end  = (uint32_t)atol(s3) + gstart[ichr];  
            nD++;
        }     
    } 
    fclose(fd);
...
int main(int argc, char **argv)
{
    ...
    if(choice<=6){  
        ...
        if(choice==0)               //BITS: two separated bs, count only
            ...
            AIListSearch(pQ, nQ, pD, nD);                      //brute force
            //dCounter(pQ, nQ, pD, nD);              
        free(pQ);
        free(pD);
    }
    else if(choice==7){ //dfName: a file folder
        func12Mi(qfName, dfName);
    }    
    else if(choice==8){ //dfName: a file folder
